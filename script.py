...
def populateWordsDict(initial_time, start_times, end_times):
...
with open(dirname + '/transcript.txt', 'r') as f:
    ...
    for row in reader:
        ...
        name = arr[1]
        content = arr[2]
        fileStartTime = toSeconds(time[1])
        break

    word_counts = populateWordsDict(fileStartTime, start_times, end_times);
    ...
    final_json = addColors(final_json);
